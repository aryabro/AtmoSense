#!/usr/bin/env python3
"""
Generate Espresso test for user login functionality using Gemini API.
...
def construct_prompt(login_activity_code, layout_xml_code, main_activity_snippet):
    ...
    prompt = f"""You are an expert Android test engineer specializing in Espresso instrumented tests. Your task is to generate a complete, production-ready Espresso test class for the user login functionality.
...
### Key Information:
- **Focal Method**: `signIn()` (lines 64-124 in LoginActivity.java)
...
1. **Class Name**: `LoginTest`
2. **Test Method Name**: `checkUserLogin` (EXACTLY this name - grading requirement)
3. **Package**: `edu.uiuc.cs427app`
4. **Location**: app/src/androidTest/java/edu/uiuc/cs427app/LoginTest.java
...
3. Include `@Before` method named `setUp()` that:
   ...
   - Adds a test account to AccountManager for testing
...
### Test Method Requirements (`checkUserLogin`):
...
def main():
    print("=" * 70)
    print("Gemini Login Test Generator for CS427 Iteration 5")
    print("=" * 70)
    print()
    ...
    output_test_path = project_root / "app/src/androidTest/java/edu/uiuc/cs427app/LoginTest.java"
    ...
    print(f"Test Class: LoginTest")
    print(f"Test Method: checkUserLogin")
    print(f"\nNext Steps:")
